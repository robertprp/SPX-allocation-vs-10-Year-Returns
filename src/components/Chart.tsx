...
import {
  ...
  Title,
  Tooltip,
  Legend, LogarithmicScale, TimeSeriesScale, TimeScale,
} from 'chart.js';
...
export default function AllocationChart({
  ...
  const returnsData = chartData
      ...
      .map((i) => ({x: new Date(i.date).getTime(), y: parseFloat((i.return_10 * 100).toFixed(2))}));
  const allocationData = chartData.map((i) => ({
    x: new Date(i.date).getTime(),
    y: i.percentage !== 0 ? parseFloat((i.percentage * 100).toFixed(2)) : undefined,
  }));
